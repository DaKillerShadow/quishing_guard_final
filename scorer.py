...
def analyze_url(url: str, blocklisted: bool = False, allowlisted: bool = False):
    ...
    checks.append({
        ...
        "message": puny_msg, "metric": "",
        "score": W_PUNYCODE if is_puny else 0, "triggered": is_puny
    })
    ...
    checks.append({
        ...
        "score": W_DGA_ENTROPY if entropy_result.is_dga else 0, "triggered": entropy_result.is_dga
    })
    ...
    checks.append({
        ...
        ) if sld_hit else "Domain name contains no suspicious keywords. ✓",
    ...
    checks.append({
        ...
        "message": f"QR code uses the shortener '{etld1}' to hide destination." if short_hit else "No URL shortener detected. ✓",
    ...
    if ("suspicious_tld" in triggered_names 
            ...
            and not blocklisted and not allowlisted):
    ...
    if blocklisted: assessment_text = "Known Malicious Domain. Blocked by Administrator."
    elif allowlisted: assessment_text = "Trusted Domain. Approved by Administrator."

    return {
        "url": url, "resolved_url": target_url, "risk_score": risk_score,
        "risk_label": risk_label, "checks": checks, "redirect_chain": chain,
        "hop_count": total_hops, "overall_assessment": assessment_text
    }