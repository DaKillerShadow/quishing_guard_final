...
// FIX: Removed 'import ../about/about_screen.dart' — AboutScreen is now
// reached via go_router (/about) so no direct import is needed here.
...
class _State extends ConsumerState<SettingsScreen> {
    ...
    return Scaffold(
      ...
      body: ListView(children: [
            ...
            child: Row(children: [
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  ref.read(apiServiceProvider).updateBaseUrl(_apiCtrl.text.trim());
        ...
        _Group(label: 'Admin Panel', children: [
          if (_isAdmin) ...[
            _Row(
              icon: '🛡', title: 'Admin session active',
            ...
            _Row(
              icon: '📊', title: 'Open Admin Dashboard',
              ...
              child: Column(children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(_loggingIn ? '⏳ Logging in…' : '🔐  LOGIN AS ADMIN'),
                  ),
                ),
              ]),
            ),
        ...
        _Group(label: 'Learning & Notifications', children: [
          _Row(
            icon: '📚', title: 'Auto-show micro-lessons',
            subtitle: 'Show a lesson after high-risk scans',
            trailing: Switch(
              value: _autoLesson, activeColor: AppColors.arc,
              onChanged: (v) {
          ...
          _Row(
            icon: '🔔', title: 'Scan notifications',
            subtitle: 'Alert when a cached link is newly flagged',
            trailing: Switch(
              value: _notifications, activeColor: AppColors.arc,
              onChanged: (v) {
        ...
        _Group(label: 'About', children: [
          _Row(
            ...
            trailing: TextButton(
              ...
              // FIX: was Navigator.push(MaterialPageRoute(...)) which bypassed
              // go_router and created a parallel navigation stack. Now uses
              // context.push('/about') consistently with all other screens.
          ...
          ...{
            ...
            // FIX: was hardcoded '2.0.0' — now reads from AppConstants so it
            // stays in sync with app_constants.dart and pubspec.yaml.
            ...
            icon: '', title: e.key, subtitle: '',
            trailing: Text(e.value, style: const TextStyle(
              fontFamily: 'monospace', fontSize: 11, color: AppColors.textColor)),
          )),
        ]),
  ...
  Future<void> _adminLogin() async {
    ...
    try {
      final token = await ref.read(apiServiceProvider)
          .adminLogin(_userCtrl.text.trim(), _passCtrl.text.trim());
      if (token != null) {
        setState(() { _loggingIn = false; _isAdmin = true; });
        _passCtrl.clear();
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(
